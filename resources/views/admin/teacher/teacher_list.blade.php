...
<div class="row">
    ...
    <?php 
        ...
        if ($info !== null) {
            $user_image = isset($info->photo) ? 'uploads/user-images/' . $info->photo : 'uploads/user-images/thumbnail.png';
        } else {
    ...
    ?>
                      ...
                      <tr>
                        ...
                        <td>
                          <div class="dAdmin_info_name min-w-250px">
                          <p><span>{{ get_phrase('Phone') }}:</span> {{ isset($info->phone) ? $info->phone : '' }}</p>

                            <p>
                             <span>{{ get_phrase('Address') }}:</span> {{ isset($info->address) ? $info->address : '' }}
...
<div class="table-responsive teacher_list display-none-view" id="teacher_list">
  <table class="table eTable eTable-2">
    ...
    <tbody>
        ...
        <?php 
          $info = json_decode($teacher->user_information);
$user_image = 'uploads/user-images/thumbnail.png'; // Default image path

if (!empty($info) && isset($info->photo)) {
