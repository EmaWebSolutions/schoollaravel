    ...
    <form method="POST" enctype="multipart/form-data" class="d-block ajaxForm" action="{{ route('admin.update', ['id' => $user->id]) }}">
        ...
        <div class="form-row">
            ...
            <div class="fpb-7">
                ...
                <input type="text" class="form-control eForm-control inputDate" id="birthday" name="birthday" value="{{ $info->birthday ?? '' }}" />
            ...
            <div class="fpb-7">
                ...
                <select name="gender" id="gender" class="form-select eForm-select eChoice-multiple-with-remove"  required>
                   ...
                   <option value="Male" {{ $info && $info->gender == 'Male' ? 'selected' : '' }}>
    {{ get_phrase('Male') }}
</option>
<option value="Female" {{ $info && $info->gender == 'Female' ? 'selected' : '' }}>
    {{ get_phrase('Female') }}
</option>
<option value="Others" {{ $info && $info->gender == 'Others' ? 'selected' : '' }}>
    {{ get_phrase('Others') }}
</option>
            ...
            <div class="fpb-7">
                ...
                <select name="blood_group" id="blood_group" class="form-select eForm-select eChoice-multiple-with-remove">
                    ...
                    <option value="a+" {{ $info && $info->blood_group == 'a+' ? 'selected' : '' }}>
    {{ get_phrase('A+') }}
</option>
<option value="a-" {{ $info && $info->blood_group == 'a-' ? 'selected' : '' }}>
    {{ get_phrase('A-') }}
</option>
<option value="b+" {{ $info && $info->blood_group == 'b+' ? 'selected' : '' }}>
    {{ get_phrase('B+') }}
</option>
<option value="b-" {{ $info && $info->blood_group == 'b-' ? 'selected' : '' }}>
    {{ get_phrase('B-') }}
</option>
<option value="ab+" {{ $info && $info->blood_group == 'ab+' ? 'selected' : '' }}>
    {{ get_phrase('AB+') }}
</option>
<option value="ab-" {{ $info && $info->blood_group == 'ab-' ? 'selected' : '' }}>
    {{ get_phrase('AB-') }}
</option>
<option value="o+" {{ $info && $info->blood_group == 'o+' ? 'selected' : '' }}>
    {{ get_phrase('O+') }}
</option>
<option value="o-" {{ $info && $info->blood_group == 'o-' ? 'selected' : '' }}>
    {{ get_phrase('O-') }}
            ...
            <div class="fpb-7">
                ...
                <textarea class="form-control eForm-control" id="address" name="address" rows="5" required>
    {{ $info ? $info->address : '' }}
</textarea>
