            ...
            <div class="table-responsive">
              <table class="table eTable eTable-2">
                ...
                <tbody>
                    ...
                    <?php 
                    $user_image = 'uploads/user-images/thumbnail.png'; // Default image

if (!empty($admin->user_information)) {
    $info = json_decode($admin->user_information);

    if ($info !== null && isset($info->photo)) {
        $user_image = 'uploads/user-images/' . $info->photo;
                    ...
                    ?>
                      <tr>
                        ...
                        <td>
                          <div class="dAdmin_info_name min-w-250px">
                            <p><span>{{ get_phrase('Phone') }}:</span>
    @if (isset($info) && isset($info->phone))
        {{ $info->phone }}
    @else
        {{ get_phrase('Phone not available') }}
    @endif
</p>

                            <p>
                             <p><span>{{ get_phrase('Address') }}:</span>
    @if (isset($info) && isset($info->address))
        {{ $info->address }}
    @else
        {{ get_phrase('Address not available') }}
    @endif
</p>
...
if (!empty($admin->user_information)) {
    ...
    if ($info !== null && property_exists($info, 'photo')) {
        $user_image = 'uploads/user-images/' . $info->photo;
    }
}
        ...
        ?>
          <tr>
            ...
            <td>
              <div class="dAdmin_info_name min-w-250px">
                <p><span>{{ get_phrase('Phone') }}:</span>
    @if (isset($info) && !is_null($info->phone))
        {{ $info->phone }}
    @else
        {{ get_phrase('Phone not available') }}
    @endif
...
@if (isset($info) && !is_null($info) && property_exists($info, 'address'))
    {{ $info->address }}
@else
    {{ get_phrase('Address not available') }}
@endif
