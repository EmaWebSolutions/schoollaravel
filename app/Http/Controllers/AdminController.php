...
class AdminController extends Controller
{
    ...
    public function dailyAttendanceFilter(Request $request)
    {
        ...
        $attendance_of_students = DailyAttendances::whereBetween('timestamp', [$first_date, $last_date])->where(['class_id' => $data['class_id'], 'section_id' => $data['section_id'],  'session_id' => $active_session])->get()->toArray();

        $no_of_users = DailyAttendances::where(['class_id' => $data['class_id'], 'section_id' => $data['section_id'],  'session_id' => $active_session])->distinct()->count('student_id');
...
if ($school) {
    $school_currency = $school->toArray();
} else {
    // Handle the case where no school record was found.
    // You can set $school_currency to a default value or take appropriate action.
    $school_currency = []; // Default empty array
}
